...
const Assessment = () => {
  ...
  const sortedData = [...students].sort((a, b) => {
    const aValue = sortColumn === "name" ? a.studentId?.name : a.studentId?.studentId;
    const bValue = sortColumn === "name" ? b.studentId?.name : b.studentId?.studentId;
    ...
    <div className="ml-[40px] mt-0 bg-whitesmoke h-[700px] flex flex-col items-center justify-start px-[30px] pb-[30px] pt-10">
      <div className="w-[1250px] bg-theme-white-default">
        <div className="h-[60px] bg-theme-white-default relative flex items-center px-4">
          <div className="text-slate-600 font-inter font-medium text-2xl leading-6 tracking-normal">SAP API 
          </div>
          ...
          {showDatePicker && (
            <div className="absolute top-[60px] left-[500px] z-10" ref={datePickerRef}>
              <DateRange
  ...
  className={`ml-4 rounded-full w-[55px] h-10  flex items-center justify-center ${
    !dateRange[0].startDate && !dateRange[0].endDate
      ? "bg-white text-text-body-light border border-slate-600" // Blue background when showing all data
      : "bg-theme-primary-dark text-theme-white-default" // White background when a date range is selected
  }`}
          ...
          <Table>
            <TableHead className="bg-indigo-100">
              <TableRow>
                ...
                <TableCell align="center" onClick={() => handleSort("studentId")}>
              ...
              {filteredData
                ...
                .map((user, index) => (
                  <TableRow key={`${user.studentId.studentId}-${index}`}>
                    ...
                    <TableCell align="center">{user.studentId.studentId}</TableCell>
                    <TableCell align="center">{user.studentId.name}</TableCell>
                    <TableCell align="center">
                      <audio controls key={`${user.studentId.studentId}-${index}`}>
                        <source src={user.s3Url} type="audio/mp4" />
                        <source src={user.s3Url.replace(".m4a", ".mp3")} type="audio/mpeg" />
                        Your browser does not support the audio element.
                      </audio>
                    </TableCell>
                    <TableCell align="center">Dams</TableCell>
                    <TableCell align="center">{responseTime}</TableCell>
                    <TableCell align="center">{format(new Date(user.createdAt), "dd/MM/yyyy HH:mm a")}</TableCell>
                    <TableCell>
